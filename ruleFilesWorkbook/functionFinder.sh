...
first_line=$(pattern -f ../patterns/cad64 /tmp/$1.tonicClosing | head -1 | awk '{print $6}')
second_line=$(pattern -f ../patterns/cad64 /tmp/$1.tonicClosing | head -1| awk '{print $9}')
awk -v first=$first_line -v second=$second_line '{ if (NR == first)
print $0,"\011""D (cadential 64)"
else
	print $0 }' /tmp/$1.tonicClosing > /tmp/$1.cad64.$$

third_line=$(pattern -f ../patterns/cad64 /tmp/$1.tonicClosing | tail -1 | awk '{print $6}')
fourth_line=$(pattern -f ../patterns/cad64 /tmp/$1.tonicClosing | tail -1 | awk '{print $9}')
awk -v third=$third_line -v fourth=$fourth_line '{ if (NR == first)
print $0,"\011""D (cadential 64)"
else
	print $0 }' /tmp/$1.cad64.$$ > /tmp/$1.cad64_a

# ### Diminished  as Dominant (first fule of V chord section)
awk '{ if ($1 ~ /^.*viio/)
 print $1,"\011"$2,"\011""D (vii as dom.)"
else print $0 }' /tmp/$1.cad64_a > /tmp/$1.dominantsubs
...
# echo "Now doing tonic expansion"
...
awk '{if ($1 ~ /[0-9]\.[iI]/ && $3 == "") print $1"\011"$2"\011""T"; else if ($1 ~ /[0-9][iI]/ && $3 == "") print $1"\011"$2"\011""T";
 else print $0}' /tmp/$1.dominantsubs_vii > /tmp/$1.smoothing
